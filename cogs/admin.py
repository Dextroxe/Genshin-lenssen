...
class Admin(commands.Cog):
    ...
    @app_commands.choices(area=[Choice(name='current_server', value=0), Choice(name='global_server', value=1)])
    async def sync(self, interaction: discord.Interaction, area: int = 0):
        if area == 0: # Copy global commands and synchronize to the current server without waiting
            self.bot.tree.copy_global_to(guild=interaction.guild)
            result = await self.bot.tree.sync(guild=interaction.guild)
        else: # Sync to the global domain, wait for an hour
        ...
        log.info(f'[instruction][Admin]sync(area={area}): {msg}')
    ...
    async def broadcast(self, interaction: discord.Interaction, message: str):
        ...
        for guild in self.bot.guilds:
            # Find the first available channel to post a message
            for channel in guild.text_channels:
                if channel.permissions_for(guild.me).send_messages:
                    try:
                        await channel.send(message)
                    except Exception as e:
                        log.error(f'[exception][Admin]broadcast: Failed to send message to channel [server]{guild} [Exceptions]{e}')
                        continue
                    else:
                        count += 1
                        break
        ...
        if option == 0:
            if param != None:
                try:
                    ...
                    await interaction.response.send_message(f'Instruction set {param} reload complete')
            else:
                ...
                try:
                    ...
                    await interaction.response.send_message('All instruction set reload complete')
        # Change presence string
        elif option == 1:
            self.presence_string = param.split(',')
            await interaction.response.send_message(f'Presence list has been changed to:{self.presence_string}')
    ...
    async def __hasAppCmdScope(self, guild: discord.Guild) -> bool:
        try:
            ...
            log.error(f'[exception][Admin]Admin > __hasAppCmdScope: [server]{guild} [Exceptions]{e}')
            return False
        else:
            return True
