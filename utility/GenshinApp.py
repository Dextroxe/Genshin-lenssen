...
class GenshinApp:
    ...
    async def setCookie(self, user_id: str, cookie: str) -> str:
        ...
        try:
            ...
            if len(accounts) == 0:
                log.info(f'[News][{user_id}]setCookie: There are no roles in the account')
                ...
                log.info(f'[News][{user_id}]setCookie: Cookie set successfully')
    ...
    async def setUID(self, user_id: str, uid: str, *, check_uid: bool = False) -> str:
        ...
        `str`: Reply to message to user
        """
        log.info(f'[instruction][{user_id}]setUID: uid={uid}, check_uid={check_uid}')
        if not check_uid:
        ...
        try:
            ...
            if int(uid) in [account.uid for account in accounts]:
                ...
                log.info(f'[News][{user_id}]setUID: Could not find character profile for this UID')
    ...
    async def claimDailyReward(self, user_id: str, *, honkai: bool = False, schedule = False) -> str:
        ...
        game_name = {genshin.Game.GENSHIN: 'Genshin', genshin.Game.HONKAI: 'Honaki 3'}
    ...
    async def getTravelerDiary(self, user_id: str, month: int) -> Union[str, discord.Embed]:
        ...
        try:
            ...
            log.error(f'[exception][{user_id}]getTravelerDiary: [retcode]{e.retcode} [exception]{e.original}')
            result = e.original
        except Exception as e:
            log.error(f'[exception][{user_id}]getTravelerDiary: [exception]{e}')
            result = f'{e}'
        else:    
            d = diary.data
            result = discord.Embed(
                title=f"{diary.nickname}'s Notes(month :{month}) ",
            ...
            result.add_field(
                ...
                value=f'{emoji.items.primogem}Primogems :{d.current_primogems} ({round(d.current_primogems/160)}{emoji.items.intertwined_fate}) <> last month:{d.last_primogems} ({round(d.last_primogems/160)}{emoji.items.intertwined_fate})\n'
                    f'{emoji.items.mora}Mora: {format(d.current_mora, ",")} <> last month: {format(d.last_mora, ",")}',
                inline=False
            )
    ...
    async def getRecordCard(self, user_id: str) -> Union[str, Tuple[genshin.models.RecordCard, genshin.models.PartialGenshinUserStats]]:
        ...
        try:
            ...
            log.error(f'[exception][{user_id}]getRecordCard: [retcode]{e.retcode} [exception]{e.original}')
            return e.original
        except Exception as e:
    ...
    async def getCharacters(self, user_id: str) -> Union[str, Sequence[genshin.models.Character]]:
        ...
        try:
            ...
            log.error(f'[exception][{user_id}]getCharacters: [retcode]{e.retcode} [exception]{e.original}')
            return e.original
        except Exception as e:
    ...
    def checkUserData(self, user_id: str, *, checkUID = True, update_use_time = True) -> Tuple[bool, str]:
        ...
        if user_id not in self.__user_data.keys():
            ...
            if checkUID and 'uid' not in self.__user_data[user_id].keys():
                log.info(f'[News][{user_id}]checkUserData: Character UID not found')
    ...
    def deleteExpiredUserData(self) -> None:
        ...
        for user_id in user_data.keys():
            if user_last_use_time.checkExpiry(user_id, now, 30) == True:
                ...
                count += 1
        log.info(f'[News][System]deleteExpiredUserData: {len(user_data)} users checked, deleted {count} expired users')
    ...
    def parseAbyssOverview(self, abyss: genshin.models.SpiralAbyss) -> discord.Embed:
        ...
        """
        result = discord.Embed(description=f'Abyss cycle {abyss.season} with Date: {abyss.start_time.astimezone().strftime("%Y.%m.%d")} ~ {abyss.end_time.astimezone().strftime("%Y.%m.%d")}', color=0x6959c1)
    ...
    def parseCharacter(self, character: genshin.models.Character) -> discord.Embed:
        ...
        embed.add_field(name=f'★{weapon.rarity} {weapon.name}', inline=True, value=f'refinement lvl: {weapon.refinement}\n Weapon lvl: {weapon.level}')

        if character.constellation > 0:
            number = {1: 'First', 2: 'Second', 3: 'Third', 4: 'Fourth', 5: 'Fifth', 6: 'Sixth'}
            msg = '\n'.join([f'Constellation: {number[constella.pos]} <> Name: {constella.name}' for constella in character.constellations if constella.activated])
    ...
    def __parseNotes(self, notes: genshin.models.Notes, shortForm: bool = False) -> str:
        ...
        result += f'{emoji.notes.resin} Recuperation  {recover_time}\n'
        # daily, weekly
        if not shortForm:
        ...
        if not shortForm:
            ...
            for expedition in notes.expeditions:
                ...
                if expedition.finished:
                    ...
                    exped_msg += f' expeditions is : {day_msg} {expedition.completion_time.strftime("%H:%M")}\n'
            result += f'Right now {exped_finished} expeditions are completed\n'
    ...
    def __getGenshinClient(self, user_id: str) -> genshin.Client:
        ...
        if uid != None and uid[0] in ['1', '2', '5']:
            client = genshin.Client(region=genshin.Region.OVERSEAS, lang='en-us')
        else:
            client = genshin.Client(lang='en-us')
